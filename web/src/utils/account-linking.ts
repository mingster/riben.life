 ...
 *
 * Uses an explicit transaction timeout to avoid P2028 (Transaction not found) in
 * production when the migration runs longer than the default timeout (e.g. under load).
 *
...
export async function linkAnonymousAccount(
	...
	try {
		await sqlClient.$transaction(
			...
			{
				timeout: 30_000, // 30s: migration does many steps; avoid P2028 (Transaction not found)
				maxWait: 10_000, // 10s: wait for a connection from the pool
			},
		);
	} catch (error) {
