...
export const getDashboardStatsAction = adminActionClient
	.metadata({ name: "getDashboardStats" })
	.schema(getDashboardStatsSchema)
	.action(async () => {
		logger.info("getDashboardStatsAction", {
			tags: ["notification", "dashboard", "stats"],
		});
		...
		if (processedDeliveries.length > 0) {
			...
			avgProcessingTime =
				totalProcessingTime / processedDeliveries.length / 1000; // Convert to seconds
		}
