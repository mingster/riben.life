...
export function CellAction({ data, onDeleted }: CellActionProps) {
	...
	const handleDelete = async () => {
		try {
			...
			if (result?.serverError) {
				...
				toastSuccess({ title: t("announcement_deleted"), description: "" });
				onDeleted?.(data.id);
			}
		} catch (error: unknown) {
			toastError({
	...
	const handleCopy = async () => {
		try {
			...
			toastSuccess({ title: t("copy"), description: data.id });
		} catch (error: unknown) {
			toastError({
				title: t("error_title"),
				description: error instanceof Error ? error.message : String(error),
