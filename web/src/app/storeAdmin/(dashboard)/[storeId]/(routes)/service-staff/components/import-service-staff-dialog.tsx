...
interface ImportServiceStaffDialogProps {
	onImported?: () => void;
}

export function ImportServiceStaffDialog({
	onImported,
}: ImportServiceStaffDialogProps) {
	...
	const handleImport = async () => {
		...
		try {
			...
			const res = await fetch(
				`/api/storeAdmin/${params.storeId}/service-staff/import`,
				{
					method: "POST",
					headers: {
	...
	return (
		<Dialog open={open} onOpenChange={handleOpenChange}>
			...
			<DialogContent className="max-w-[calc(100%-1rem)] p-4 sm:p-6 sm:max-w-md max-h-[calc(100vh-2rem)] overflow-y-auto">
				<DialogHeader>
					<DialogTitle>{t("import") || "Import Service Staff"}</DialogTitle>
