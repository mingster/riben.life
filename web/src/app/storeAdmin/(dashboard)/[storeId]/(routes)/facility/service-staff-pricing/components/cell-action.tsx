...
import { deleteFacilityServiceStaffPricingRuleAction } from "@/actions/storeAdmin/facility-service-staff-pricing/delete-facility-service-staff-pricing-rule";
...
interface CellActionProps {
	data: FacilityServiceStaffPricingRuleColumn;
...
export const CellAction: React.FC<CellActionProps> = ({
	...
	const onConfirm = async () => {
		try {
			setLoading(true);
			const result = await deleteFacilityServiceStaffPricingRuleAction(
			...
			toastSuccess({
				title: t("facility_service_staff_pricing_rule_deleted"),
				description: "",
			});
			onDeleted?.(data.id);
		} catch (error: unknown) {
	...
	const onCopy = (id: string) => {
		...
		toastSuccess({
			title: "Pricing rule ID copied to clipboard.",
			description: "",
		});
	};
