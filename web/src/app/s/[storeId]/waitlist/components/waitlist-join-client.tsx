...
/** Keys used by FormPhoneOtp for saved phone (country code + local number). */
const PHONE_COUNTRY_CODE_KEY = "phone_country_code";
const PHONE_LOCAL_NUMBER_KEY = "phone_local_number";

/**
 * Build full E.164-style phone from FormPhoneOtp localStorage (same keys as form-phone-otp.tsx).
 * Returns null if nothing saved or invalid.
 */
...
interface WaitlistJoinClientProps {
  ...
  /** Pre-fill phone when user is signed in (from profile). */
...
export function WaitlistJoinClient({
  ...
  // Pre-fill phone from FormPhoneOtp localStorage when no server prefill and client is hydrated
  useEffect(() => {
